...
import { userStorage, adminStorage } from '@/utils/storage'
...
service.interceptors.request.use(
  config => {
    ...
    const adminToken = adminStorage.get('adminToken')
...
async function handleTokenExpired(isAdminRequest) {
  ...
  // 用户 token 过期：尝试刷新
  const refreshToken = userStorage.get('refreshToken')
  if (!refreshToken) {
    clearUserAuth()
    return
  }
  try {
    ...
    if (res.data.code === 200) {
      userStorage.set('token', res.data.data.accessToken)
      return
    }
  } catch {
    // refresh failed
